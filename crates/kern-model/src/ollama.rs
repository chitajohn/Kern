//! Ollama chat API adapter (ARCHITECTURE.md §8.2, decision D10).
//!
//! Raw `reqwest` call — no SDK. Environment: `OLLAMA_BASE_URL` (optional;
//! default `http://localhost:11434`), no key required (local inference).
//!
//! Provider-specific translation performed here:
//! - Ollama does not assign tool-call ids; the runtime synthesizes
//!   deterministic `ollama-<index>` ids (per response position). They are
//!   unique within the conversation and stable across a crash-replay of the
//!   same recorded response.
//! - Tool results use the plain `tool` message role (Ollama does not key them
//!   by id).

use async_trait::async_trait;
use serde_json::{json, Value};

use crate::error::{from_status, ModelError};
use crate::provider::ModelProvider;
use crate::types::{CompletionRequest, CompletionResponse, FinishReason, Message, Role, ToolCall};

pub const DEFAULT_BASE_URL: &str = "http://localhost:11434";
/// Guard against unbounded provider responses.
const MAX_RESPONSE_BYTES: u64 = 10 * 1024 * 1024;

pub struct OllamaProvider {
    client: reqwest::Client,
    base_url: String,
}

impl OllamaProvider {
    pub fn new(base_url: impl Into<String>) -> Self {
        Self {
            client: reqwest::Client::new(),
            base_url: base_url.into(),
        }
    }

    /// Build from the environment (`OLLAMA_BASE_URL`).
    pub fn from_env() -> Self {
        let base_url =
            std::env::var("OLLAMA_BASE_URL").unwrap_or_else(|_| DEFAULT_BASE_URL.to_string());
        Self::new(base_url)
    }
}

#[async_trait]
impl ModelProvider for OllamaProvider {
    fn id(&self) -> &str {
        "ollama"
    }

    async fn complete(&self, req: &CompletionRequest) -> Result<CompletionResponse, ModelError> {
        let body = build_body(req);
        let url = format!("{}/api/chat", self.base_url.trim_end_matches('/'));

        let response = self
            .client
            .post(&url)
            .json(&body)
            .send()
            .await
            .map_err(|e| ModelError::Unavailable(format!("ollama transport error: {e}")))?;

        if !response.status().is_success() {
            return Err(from_status(response.status(), "ollama"));
        }
        if let Some(len) = response.content_length() {
            if len > MAX_RESPONSE_BYTES {
                return Err(ModelError::InvalidResponse(format!(
                    "ollama response exceeds {MAX_RESPONSE_BYTES} bytes"
                )));
            }
        }
        let payload: Value = response.json().await.map_err(|e| {
            ModelError::InvalidResponse(format!("ollama response is not JSON: {e}"))
        })?;

        normalize(&payload)
    }
}

/// Serialize a normalized request into the Ollama chat body.
fn build_body(req: &CompletionRequest) -> Value {
    let mut body = json!({
        "model": req.model,
        "stream": false,
        "messages": req.messages.iter().map(ollama_message).collect::<Vec<_>>(),
    });
    if !req.tools.is_empty() {
        body["tools"] = Value::Array(
            req.tools
                .iter()
                .map(|t| {
                    json!({
                        "type": "function",
                        "function": {
                            "name": t.name,
                            "description": t.description,
                            "parameters": t.input_schema,
                        }
                    })
                })
                .collect(),
        );
    }
    if let Some(temperature) = req.temperature {
        body["options"] = json!({ "temperature": temperature });
    }
    body
}

fn ollama_message(msg: &Message) -> Value {
    match msg.role {
        Role::System | Role::User | Role::Tool => {
            json!({ "role": msg.role_str(), "content": msg.content })
        }
        Role::Assistant => {
            let mut m = json!({ "role": "assistant", "content": msg.content });
            if !msg.tool_calls.is_empty() {
                m["tool_calls"] = Value::Array(
                    msg.tool_calls
                        .iter()
                        .map(|c| {
                            json!({
                                "function": { "name": c.name, "arguments": c.arguments }
                            })
                        })
                        .collect(),
                );
            }
            m
        }
    }
}

/// Normalize an Ollama chat response payload.
fn normalize(payload: &Value) -> Result<CompletionResponse, ModelError> {
    let message = payload.get("message").ok_or_else(|| {
        ModelError::InvalidResponse("ollama response missing message".to_string())
    })?;

    let text = message
        .get("content")
        .and_then(Value::as_str)
        .unwrap_or("")
        .to_string();

    let mut calls = Vec::new();
    if let Some(tool_calls) = message.get("tool_calls").and_then(Value::as_array) {
        for (index, raw) in tool_calls.iter().enumerate() {
            let function = raw.pointer("/function").ok_or_else(|| {
                ModelError::InvalidResponse("tool call missing function".to_string())
            })?;
            let name = function
                .get("name")
                .and_then(Value::as_str)
                .ok_or_else(|| ModelError::InvalidResponse("tool call missing name".to_string()))?
                .to_string();
            let arguments = function
                .get("arguments")
                .cloned()
                .unwrap_or(Value::Object(Default::default()));
            if !arguments.is_object() {
                return Err(ModelError::InvalidResponse(
                    "tool call arguments must be a JSON object".to_string(),
                ));
            }
            calls.push(ToolCall {
                id: format!("ollama-{index}"),
                name,
                arguments,
            });
        }
    }

    if !calls.is_empty() {
        return Ok(CompletionResponse::ToolCalls(calls));
    }

    let reason = match payload
        .get("done_reason")
        .and_then(Value::as_str)
        .unwrap_or("stop")
    {
        "stop" => FinishReason::Stop,
        "length" => FinishReason::Length,
        other => FinishReason::Other(other.to_string()),
    };
    Ok(CompletionResponse::Finish { reason, text })
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::types::ToolSpec;

    fn req() -> CompletionRequest {
        let mut r = CompletionRequest::new(
            "ollama",
            "llama3.1",
            vec![
                Message::system("You are a research assistant."),
                Message::user("Write a file."),
            ],
        );
        r.tools = vec![ToolSpec {
            name: "filesystem".to_string(),
            description: "read/write files".to_string(),
            input_schema: json!({ "type": "object" }),
        }];
        r.temperature = Some(0.2);
        r
    }

    #[tokio::test]
    async fn finish_fixture_round_trips() {
        let mut server = mockito::Server::new_async().await;
        let mock = server
            .mock("POST", "/api/chat")
            .match_body(mockito::Matcher::PartialJsonString(
                json!({
                    "model": "llama3.1",
                    "stream": false,
                    "options": { "temperature": 0.2 },
                })
                .to_string(),
            ))
            .with_status(200)
            .with_header("content-type", "application/json")
            .with_body(
                r#"{"model":"llama3.1","message":{"role":"assistant","content":"Hello!"},"done":true,"done_reason":"stop"}"#,
            )
            .create();

        let provider = OllamaProvider::new(server.url());
        let response = provider.complete(&req()).await.unwrap();
        match response {
            CompletionResponse::Finish { reason, text } => {
                assert_eq!(reason, FinishReason::Stop);
                assert_eq!(text, "Hello!");
            }
            other => panic!("expected finish, got {other:?}"),
        }
        mock.assert();
    }

    #[tokio::test]
    async fn tool_calls_fixture_synthesizes_ids() {
        let mut server = mockito::Server::new_async().await;
        let mock = server
            .mock("POST", "/api/chat")
            .with_status(200)
            .with_header("content-type", "application/json")
            .with_body(
                r#"{"model":"llama3.1","message":{"role":"assistant","content":"","tool_calls":[
                    {"function":{"name":"filesystem","arguments":{"path":"/tmp/x"}}},
                    {"function":{"name":"http","arguments":{"url":"https://example.com"}}}
                ]},"done":true,"done_reason":"stop"}"#,
            )
            .create();

        let provider = OllamaProvider::new(server.url());
        let response = provider.complete(&req()).await.unwrap();
        match response {
            CompletionResponse::ToolCalls(calls) => {
                assert_eq!(calls.len(), 2);
                assert_eq!(calls[0].id, "ollama-0");
                assert_eq!(calls[0].name, "filesystem");
                assert_eq!(calls[0].arguments["path"], "/tmp/x");
                assert_eq!(calls[1].id, "ollama-1");
                assert_eq!(calls[1].name, "http");
            }
            other => panic!("expected tool calls, got {other:?}"),
        }
        mock.assert();
    }

    #[tokio::test]
    async fn tool_results_use_plain_tool_role() {
        let mut server = mockito::Server::new_async().await;
        let mock = server
            .mock("POST", "/api/chat")
            .match_body(mockito::Matcher::PartialJsonString(
                json!({ "messages": [
                    { "role": "system", "content": "You are a research assistant." },
                    { "role": "user", "content": "Write a file." },
                    { "role": "assistant", "content": "", "tool_calls": [
                        { "function": { "name": "filesystem", "arguments": { "path": "/tmp/x" } } }
                    ]},
                    { "role": "tool", "content": "ok" }
                ] }).to_string(),
            ))
            .with_status(200)
            .with_header("content-type", "application/json")
            .with_body(r#"{"message":{"role":"assistant","content":"done"},"done":true,"done_reason":"stop"}"#)
            .create();

        let mut r = req();
        r.messages.push(Message::assistant_with_tool_calls(
            "",
            vec![ToolCall {
                id: "ollama-0".into(),
                name: "filesystem".into(),
                arguments: json!({ "path": "/tmp/x" }),
            }],
        ));
        r.messages.push(Message::tool_result("ollama-0", "ok"));

        let provider = OllamaProvider::new(server.url());
        provider.complete(&r).await.unwrap();
        mock.assert();
    }

    #[tokio::test]
    async fn length_done_reason_maps_to_length() {
        let mut server = mockito::Server::new_async().await;
        server
            .mock("POST", "/api/chat")
            .with_status(200)
            .with_header("content-type", "application/json")
            .with_body(
                r#"{"message":{"role":"assistant","content":"partial"},"done":true,"done_reason":"length"}"#,
            )
            .create();
        let provider = OllamaProvider::new(server.url());
        let response = provider.complete(&req()).await.unwrap();
        match response {
            CompletionResponse::Finish { reason, .. } => {
                assert_eq!(reason, FinishReason::Length);
            }
            other => panic!("expected finish, got {other:?}"),
        }
    }

    #[tokio::test]
    async fn status_codes_map_to_errors() {
        for (status, expected) in [(404, "unavail"), (500, "unavail")] {
            let mut server = mockito::Server::new_async().await;
            server
                .mock("POST", "/api/chat")
                .with_status(status)
                .with_body("{}")
                .create();
            let provider = OllamaProvider::new(server.url());
            let err = provider.complete(&req()).await.unwrap_err();
            assert!(
                matches!(err, ModelError::Unavailable(_)),
                "status {status}: {err:?}"
            );
            let _ = expected;
        }
    }

    #[tokio::test]
    async fn malformed_body_is_invalid_response() {
        let mut server = mockito::Server::new_async().await;
        server
            .mock("POST", "/api/chat")
            .with_status(200)
            .with_header("content-type", "application/json")
            .with_body(r#"{"not": "a valid chat response"}"#)
            .create();
        let provider = OllamaProvider::new(server.url());
        let err = provider.complete(&req()).await.unwrap_err();
        assert!(matches!(err, ModelError::InvalidResponse(_)));
    }
}
