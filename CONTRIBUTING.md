...
- **Never log secrets.** Provider keys are read from the environment and redacted at every
  log layer. `crates/kern-cli/tests/redaction.rs` audits this; keep it green.
...
4. **Update the docs that describe what you changed.** `ARCHITECTURE.md` (design decisions —
   document the decision when the change is consequential), `SPEC.md` (normative contracts:
   events, API, config, checkpoint format), or `README.md` (user-facing claims). Never mark
...
- **Event catalog:** kinds are catalog-pinned in `SPEC.md §7`. Adding a kind is fine;
  changing the meaning of an existing kind is a breaking change.
- **The engine loop:** effectively-once tool semantics (`ARCHITECTURE.md §7.2`) and restore
  behavior (`§11.3`) are load-bearing. Read them before editing `engine.rs`.
